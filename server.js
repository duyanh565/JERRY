...
app.post('/api/patch-file', (req, res) => {
  const { key: keyVal, patchIds, section, fileName, fileBase64 } = req.body;
  if (!keyVal)     return res.status(400).json({ error: 'Thiếu mã key' });
  if (!fileBase64) return res.status(400).json({ error: 'Thiếu file cần patch' });
