...
// FIX TRIỆT ĐỂ: Hàm vá xương thông minh bảo toàn cấu trúc tệp giúp giữ tâm đỏ
function applyBones(buf, bonesToPatch = []) {
  // Nếu mảng rỗng, áp dụng toàn bộ danh sách xương
  ...
  for (const bone of targetBones) {
    ...
    // KHÔNG ĐƯỢC XÓA XƯƠNG GỐC CÓ TRONG HEX CONFIG (Tránh lỗi mất tâm đỏ)
    if (bone === 'bone_Hips' || bone === 'bone_Spine1') continue;

    const search = Buffer.from(bone, 'latin1');
    
    // FIX TÂM ĐỎ: Đặt byte đầu là 0x00 để kết thúc chuỗi tên xương, 
    // các byte sau giữ nguyên ký tự cũ để tránh làm hỏng cấu trúc dữ liệu liền kề
    const replace = Buffer.from(search);
    replace[0] = 0x00; 
...
function resolveKey(keyVal) {
  ...
  if (!key)        return { ok: false, status: 403, error: 'Mã khóa không tồn tại!' };
  if (!key.active) return { ok: false, status: 403, error: 'Giấy phép bị khóa!' };
...
app.post('/api/validate-key', (req, res) => {
  ...
  if (deviceId && typeof deviceId === 'string' && deviceId.startsWith('MAKEDATA-')) {
    if (key.bound_device_id && key.bound_device_id !== deviceId) {
      return res.status(403).json({ success: false, error: 'Key đã đăng ký cho thiết bị khác!' });
    } else if (!key.bound_device_id) {
  ...
  if (!exists) {
    if (devs.length >= key.max_devices) return res.status(403).json({ success: false, error: 'Vượt giới hạn thiết bị' });
  ...
  res.json({ success: true, type: key.type, expiry: key.expiry });
});

// ── ROUTER API PATCH FILE ─────────────────────────────────────
app.post('/api/patch-file', (req, res, next) => {
  ...
  if (section === 'avatar') {
    ...
    for (const id of patchIds) {
      if (+id === 1) {
        // Thực thi hàm applyBones bảo mật tâm đỏ mới phục vụ Aim Body Selection
  ...
  if (total === 0) return res.status(422).json({ error: 'Không tìm thấy vị trí Hex thích hợp trong file.' });
...
app.get('/api/download/:id', (req, res) => {
  ...
  if (!entry || entry.expiry < Date.now()) return res.status(404).json({ error: 'Link hết hạn' });
