...
// Migration: thêm cột bound_device_id nếu chưa có (cho DB cũ)
try {
  db.exec(`ALTER TABLE keys ADD COLUMN bound_device_id TEXT DEFAULT NULL`);
} catch (e) {
...
const CACHE_HEX_CONFIG = {
  1: [
    { find:'226e1f3f5300000000000000', replace:'226E1F3F53000000000000000000000000000000000000000000000000000000000100000059DFCA3DE48C023E000000001DBB143E' },
    { find:'bf9fb489cd0000000000000000000000', replace:'BF9FB489CD00000000000000000000000000000000000000000000000000000000010000003E11CB3D7283053E000000006D27103E' },
  ],
  3: [
...
// ── Temp download store (tránh blob URL trên iOS WKWebView) ──
const downloadStore = new Map(); // id → { buf, filename, expiry }
const DOWNLOAD_TTL_MS = 5 * 60 * 1000; // 5 phút
setInterval(() => {
  const now = Date.now();
  for (const [id, entry] of downloadStore) {
    if (entry.expiry < now) downloadStore.delete(id);
  }
}, 60_000);
...
// Xoá sạch admin cũ (khi quên cả username lẫn password)
app.get('/api/auth/factory-reset-admin', (req, res) => {
  const secret = process.env.RESET_SECRET;
  if (!secret) return res.status(503).send('Chưa cấu hình RESET_SECRET trên server');
  if (!req.query.token || req.query.token !== secret) return res.status(403).send('Mã reset sai');
  db.prepare('DELETE FROM refresh_tokens').run();
  db.prepare('DELETE FROM admins').run();
  res.send('Đã xoá admin cũ thành công. Vào /api/admin để tạo tài khoản mới.');
});

app.post('/api/auth/reset-password', async (req, res) => {
  const { reset_token, new_password } = req.body;
  const secret = process.env.RESET_SECRET;
  if (!secret) return res.status(503).json({ error: 'Chưa cấu hình RESET_SECRET trên server' });
  if (!reset_token || reset_token !== secret) return res.status(403).json({ error: 'Mã reset sai' });
  if (!new_password || new_password.length < 6) return res.status(400).json({ error: 'Mật khẩu phải ít nhất 6 ký tự' });
...
app.get('/api/keys', requireAdmin, (req, res) => {
  ...
  res.json({
    keys: rows.map(k => ({
      ...
      mu: k.max_uses, u: k.uses, md: k.max_devices, deviceCount: devMap[k.id] ?? 0,
      note: k.note, boundDeviceId: k.bound_device_id || null,
    })),
  });
});
...
// Reset bound device ID cho một key (admin)
app.put('/api/keys/:id/reset-device', requireAdmin, (req, res) => {
  const id  = +req.params.id;
  const key = db.prepare('SELECT * FROM keys WHERE id=?').get(id);
  if (!key) return res.status(404).json({ error: 'Key không tồn tại' });
  db.prepare('UPDATE keys SET bound_device_id=NULL WHERE id=?').run(id);
  addLog('Reset bound device ID', { keyVal: key.value, keyId: id, ip: getIp(req) });
  res.json({ ok: true });
});
...
app.post('/api/validate-key', (req, res) => {
  const { key: keyVal, fingerprint: clientFp, deviceId } = req.body;
  ...
  // ── Kiểm tra bound device ID (MAKEDATA-XXXXXXXXX) ───────────
  const hasDeviceId = deviceId && typeof deviceId === 'string' && deviceId.startsWith('MAKEDATA-');
  let deviceIdVerified = false; // true = đã xác nhận đúng thiết bị qua MAKEDATA-

  if (hasDeviceId) {
    if (key.bound_device_id) {
      // Key đã gắn ID — kiểm tra khớp không
      if (key.bound_device_id !== deviceId) {
        addLog('Xác thực key thất bại', { keyVal, keyId: key.id, success: 0, ip, message: 'Device ID không khớp với ID đã đăng ký' });
        return res.status(403).json({ success: false, error: 'Key này đã được đăng ký cho thiết bị khác! Liên hệ admin để được hỗ trợ.' });
      }
      // ID khớp — thiết bị đúng, bỏ qua giới hạn fingerprint
      deviceIdVerified = true;
    } else {
      // Lần đầu — gắn device ID vào key
      db.prepare('UPDATE keys SET bound_device_id=? WHERE id=?').run(deviceId, key.id);
      deviceIdVerified = true;
    }
  }

  if (deviceIdVerified) {
    // Thiết bị đã xác minh qua MAKEDATA- ID → chỉ cập nhật last_seen, không check giới hạn
    const existDev = db.prepare('SELECT id FROM devices WHERE key_id=? AND fingerprint=?').get(key.id, fp);
    if (existDev) {
      db.prepare("UPDATE devices SET last_seen=datetime('now'), ip=? WHERE id=?").run(ip, existDev.id);
    } else {
      // fingerprint mới nhưng MAKEDATA- đã khớp → cho phép, ghi lại
    ...
    // Không có MAKEDATA- ID → dùng fingerprint + giới hạn thiết bị như cũ
  ...
  res.json({ success: true, type: key.type, toolType: key.tool_type, expiry: key.expiry, boundDeviceId: key.bound_device_id || deviceId || null });
});

// ── SERVER-SIDE PATCH (hex không bao giờ ra client) ──────────
app.post('/api/patch-file', (req, res, next) => {
  ...
  let keyVal, patchIds, section, fileName, fileBuf;
  if (req.file) {
    keyVal   = req.body.key;
    section  = req.body.section;
    fileName = req.file.originalname;
    fileBuf  = req.file.buffer;
    try { patchIds = JSON.parse(req.body.patchIds); } catch { return res.status(400).json({ error: 'patchIds không hợp lệ' }); }
  } else {
    keyVal   = req.body.key;
    section  = req.body.section;
    fileName = req.body.fileName;
    patchIds = req.body.patchIds;
    const b64 = req.body.fileBase64;
    if (!b64) return res.status(400).json({ error: 'Thiếu file cần patch' });
    fileBuf = Buffer.from(b64, 'base64');
  }
  if (!keyVal)   return res.status(400).json({ error: 'Thiếu mã key' });
  if (!patchIds || !Array.isArray(patchIds) || !patchIds.length) return res.status(400).json({ error: 'Chưa chọn patch nào' });
  ...
  addLog(`Patch file (${section}, ids:${patchIds.join(',')})`, {
    keyVal, keyId: r.key.id, success: total > 0 ? 1 : 0, ip: getIp(req),
  ...
  const outName = 'mod_' + (fileName || 'output.bin');
  const dlId = randomUUID();
  downloadStore.set(dlId, { buf, filename: outName, expiry: Date.now() + DOWNLOAD_TTL_MS });

  res.json({ ok: true, downloadId: dlId, patchCount: total });
});

// ── Serve patched file by download ID ────────────────────────
app.get('/api/download/:id', (req, res) => {
  const entry = downloadStore.get(req.params.id);
  if (!entry || entry.expiry < Date.now()) {
    downloadStore.delete(req.params.id);
    return res.status(404).json({ error: 'Link tải đã hết hạn hoặc không tồn tại' });
  }
  downloadStore.delete(req.params.id);
  const safeFileName = entry.filename.replace(/[^\x20-\x7E]/g, '_').replace(/["\\]/g, '_');
  res.set({
    'Content-Type': 'application/octet-stream',
    'Content-Disposition': `attachment; filename="${safeFileName}"; filename*=UTF-8''${encodeURIComponent(entry.filename)}`,
