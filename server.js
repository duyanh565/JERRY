...
// FIX LOGIC AIM BODY: Hàm nhận thêm mảng bonesToPatch để loại trừ/chỉ định đúng bone cần sửa.
function applyBones(buf, bonesToPatch = []) {
  // Nếu không truyền hoặc truyền mảng trống, mặc định áp dụng toàn bộ (tính năng cũ) để tránh lỗi
  const targetBones = (Array.isArray(bonesToPatch) && bonesToPatch.length > 0) ? bonesToPatch : AIM_BODY_BONES;
  let count = 0;

  for (const bone of targetBones) {
    // Bảo vệ cấu trúc: Chỉ xử lý nếu bone nằm trong danh sách được định nghĩa hợp lệ
    if (!AIM_BODY_BONES.includes(bone)) continue;

    const search  = Buffer.from(bone, 'latin1');
    // Ghi đè bằng độ dài byte tương đương để KHÔNG LÀM LỆCH OFFSET của file gây lỗi file/văng game!
...
app.post('/api/validate-key', (req, res) => {
  ...
  if (deviceId && typeof deviceId === 'string' && deviceId.startsWith('MAKEDATA-')) {
...
app.post('/api/patch-file', (req, res, next) => {
  ...
  let keyVal, patchIds, section, fileName, fileBuf, selectedBones = [];
  ...
  if (req.file) {
    ...
    // Khôi phục mảng bones đã chọn từ FormData
    try { selectedBones = req.body.selectedBones ? JSON.parse(req.body.selectedBones) : []; } catch { selectedBones = []; }
  } else {
    keyVal   = req.body.key;
    section  = req.body.section;
    fileName = req.body.fileName;
    patchIds = req.body.patchIds;
    selectedBones = req.body.selectedBones || [];
  ...
  if (section === 'avatar') {
    ...
    for (const id of patchIds) {
      if (+id === 1) {
        // Áp dụng bộ lọc xương được chọn cụ thể từ Client
        total += applyBones(buf, selectedBones);
