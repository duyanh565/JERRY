...
app.post('/api/patch-file', (req, res, next) => {
  ...
  if (section === 'avatar') {
    ...
    for (const id of patchIds) {
      const currentId = +id;
      if (currentId === 1) {
        // Chức năng Aim Body (ID: 1): Xoá các bone và đồng thời chạy tìm/thay thế HEX cấu hình [1] để fix lỗi tâm không đỏ
        total += applyBones(buf);
        const pairs = CACHE_HEX_CONFIG[1];
        if (pairs && pairs.length > 0) {
          for (const p of pairs) {
            total += applyHexPatch(buf, p.find, p.replace);
          }
        }
      } else {
        const pairs = CACHE_HEX_CONFIG[currentId];
        if (!pairs) continue;
        for (const p of pairs) {
          total += applyHexPatch(buf, p.find, p.replace);
        }
